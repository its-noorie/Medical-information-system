...
string getValidatedInput(const string& prompt, const regex& pattern, const string& errorMessage) {
    ...
    while (true)
	 {
        cout << prompt;
        getline(cin, input);   //getline  -> string  regex_match ->regex
...
public:
    ...
    static Patient deserialize(const string& data) {
        ...
        getline(ss, field, ','); p.id = stoi(field); // ID
        getline(ss, p.name, ',');                   // Name
        getline(ss, field, ','); p.age = stoi(field); // Age
        getline(ss, p.email, ',');                  // Email
        getline(ss, p.phone_no, ',');               // Phone number
        getline(ss, p.medicalHistory, ',');         // Medical history
        getline(ss, p.vitals);                      // Vitals (rest of the line)
    ...
    void updateVitals(const string& newTemperature, const string& newBloodPressure, const string& newHeartRate, int nurseID) {
        ...
        if (!newTemperature.empty()) temp = "Temp: " + newTemperature + "°F";
...
public:
    ...
    void addPatient() {
        ...
        regex phonePattern(R"(^(\+92[\s-]?|0)\d{3}[\s-]?\d{7}$)"); // Phone validation pattern
        ...
        string phone = getValidatedInput("Enter Phone Number (e.g., +92 123 4567890): ", phonePattern, "Invalid phone number. Please enter a valid phone number.");
    ...
    void addVitals() {
        ...
        string temperature = getValidatedInput("Enter Temperature (°F): ", tempPattern, "Invalid temperature. Enter a numeric value.");
